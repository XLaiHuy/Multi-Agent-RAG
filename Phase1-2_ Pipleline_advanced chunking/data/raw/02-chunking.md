...
## 1. Chunking là gì và tại sao quan trọng trong RAG?

**Chunking (Phân đoạn tài liệu)** là kỹ thuật chia nhỏ các văn bản hoặc tài liệu dài thành các đoạn (chunks) có kích thước phù hợp trước khi đưa vào mô hình Embedding và lưu trữ trong Vector Database.

### Tại sao Chunking lại đóng vai trò sống còn trong kiến trúc RAG?
1. **Khắc phục giới hạn cửa sổ ngữ cảnh (Context Window):** Mô hình Embedding và LLM luôn có giới hạn số lượng token tối đa. Chunking giúp chia nhỏ dữ liệu để nhúng và truy xuất chính xác từng phần.
2. **Tăng độ chính xác khi tìm kiếm tương đồng (Similarity Search):** Một đoạn văn bản ngắn, tập trung vào một chủ đề duy nhất sẽ tạo ra vector embedding sắc nét hơn rất nhiều so với nhúng cả một cuốn sách dài (vốn làm loãng ngữ nghĩa).
3. **Giảm nhiễu (Noise Reduction):** Giúp LLM chỉ tiếp nhận đúng những đoạn thông tin liên quan trực tiếp đến câu hỏi của người dùng, tránh bị phân tâm bởi các thông tin không liên quan xung quanh.
4. **Tối ưu chi phí và độ trễ (Latency):** Chỉ gửi những chunk liên quan nhất vào Prompt của LLM giúp giảm lượng token tiêu thụ và tăng tốc độ sinh câu trả lời.

---

## 2. So sánh các chiến lược chunking
